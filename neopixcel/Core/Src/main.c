...
{
  /* USER CODE BEGIN 1 */
PixelRGB_t pixel[NUM] = {0};
  uint32_t dmaBuffer[BUFF_SIZE] = {0};
  uint32_t *pBuff;
  int i, j, k;
  uint16_t stepSize;
  /* USER CODE END 1 */
  ...
  while (1)
  {
    ...
    dmaBuffer[BUFF_SIZE - 1] = 0; // last element must be 0!

    HAL_TIM_PWM_Start_DMA(&htim2, TIM_CHANNEL_3, dmaBuffer, BUFF_SIZE);

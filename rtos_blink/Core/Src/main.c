...
//osThreadId blinkLed1TaskHandle;
/* USER CODE BEGIN PV */
...
osMessageQDef(myQueue,20,uint8_t); // define queue 
osMessageQId myQueue01Handle = osMessageCreate(osMessageQ(myQueue), NULL);
...
int main(void)
{
  ...
  osThreadDef(blinkTask, StartDefaultTask, osPriorityNormal, 0, 128);
  blinkTaskHandle = osThreadCreate(osThread(blinkTask), NULL);

  /* definition and creation of blinkLedTask */
  osThreadDef(blinkLedTask, StartTask02, osPriorityIdle, 0, 128);
  blinkLedTaskHandle = osThreadCreate(osThread(blinkLedTask), NULL);
  
  osThreadDef(blinkLed1Task, StartTask01, osPriorityIdle, 0, 128);
  blinkLed1TaskHandle = osThreadCreate(osThread(blinkLed1Task), NULL);
